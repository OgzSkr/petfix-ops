...
let memoryDbCacheAt = 0;

function invalidateMemoryDbCache() {
  memoryDbCache = null;
  memoryDbCacheAt = 0;
}

async function readDbUncached() {
...
export async function readDb() {
  const ttl = Number(limits.dbReadMemoryCacheMs) || 0;
  const now = Date.now();
  if (ttl > 0 && memoryDbCache && now - memoryDbCacheAt < ttl) {
