...
import { dedupeBarcodes, detectVariantKey, normalizeBarcode, parseWeightGrams, barcodesEquivalent } from '../normalize.js';
...
export function mergeIncomingChannelProduct(existing, incoming, masterName = null) {
  ...
  if (Array.isArray(incoming.channelBarcodes) && incoming.channelBarcodes.length) {
    merged.channelBarcodes = dedupeBarcodes([
      ...(existing?.channelBarcodes || []),
      ...incoming.channelBarcodes
    ]);
  }
...
export function mergeCatalogChannelProduct(existing, incoming) {
  ...
  merged.catalogListTypes = incoming.catalogListTypes || merged.catalogListTypes || null;
  ...
  merged.channelImageUrl = incoming.channelImageUrl || merged.channelImageUrl || null;
...
export async function ingestUberEatsCatalogProducts(options = {}) {
  ...
  const catalog = await fetchUberEatsCatalogProducts(cfg, {
    ...options,
...
export function enrichChannelProductsFromMaster(channelProducts, masterProducts) {
  ...
  for (const cp of channelProducts) {
    const master = masterProducts.find((row) => channelProductBarcodeList(cp).some(
      (code) => barcodesEquivalent(code, row.benimposBarcode)
