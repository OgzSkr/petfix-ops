...
import { listActiveMatchingSalesChannels, channelHasFeature, getChannel } from '../channels/registry.js';
...
function poolTabForChannel(channelId) {
  ...
  if (channelId === 'trendyol-marketplace') return 'trendyol-marketplace';
  if (channelId === 'woocommerce') return 'woocommerce';
  return channelId;
}
...
export function resolveInboxHref(row = {}) {
  ...
  const base = `/products/inbox?channelId=${encodeURIComponent(channelId)}`;
...
export function buildMatchingQueue(db, config = {}) {
  ...
  for (const channel of listActiveMatchingSalesChannels()) {
    ...
    channels.push({
      ...
      hrefMissing: `/products/inbox?channelId=${encodeURIComponent(channel.id)}&queueMode=missing_master`,
      hrefAutoMatched: `/products/inbox?channelId=${encodeURIComponent(channel.id)}&queueMode=high_confidence`,
      hrefReview: `/products/inbox?channelId=${encodeURIComponent(channel.id)}&queueMode=manual_review`
    });
  }
...
export function catalogChannelOpsConfig(channelId) {
  ...
  if (channelId === 'trendyol-marketplace') {
    return {
      ...
      catalogLabel: 'Trendyol katalog sync (Ürün Ayarları)',
      catalogDetail: 'Paneldeki Trendyol ürün listesinden havuza aktarır'
    };
  }
  if (channelId === 'woocommerce') {
    return {
      ...
      label: channel.label,
      steps: ['master', 'catalog', 'auto-match'],
      catalogLabel: 'WooCommerce katalog sync',
      catalogDetail: 'petfix.com.tr mağaza ürünlerini havuza çeker'
    };
  }
  if (channelId === 'yemeksepeti') {
    return {
