...
export const DEFAULT_MATCHING_SYNC_CHANNELS = [...HZLMRKTOPS_BUYBOX_CHANNEL_IDS];
...
const ALLOWED_SYNC_CHANNELS = new Set(HZLMRKTOPS_BUYBOX_CHANNEL_IDS);
...
export function normalizeMatchingSyncSettings(input = {}, platformEnv = {}) {
  ...
  const envEnabled = String(
    envValue(process.env, platformEnv, 'MATCHING_SYNC_ENABLED', '')
  ).toLowerCase();
  const envInterval = Number(envValue(process.env, platformEnv, 'MATCHING_SYNC_INTERVAL_MINUTES', '1440'));
  const matchingEnabled = String(
    envValue(process.env, platformEnv, 'PRODUCT_MATCHING_ENABLED', 'false')
  ).toLowerCase();
  ...
  const enabledDefault = matchingEnabled === 'true' || matchingEnabled === '1';
...
export function catalogStepsForChannel(channelId, { uberIncludeOrders = false, matchingEnabled = true } = {}) {
  if (!matchingEnabled) {
    if (channelId === 'uber-eats' && uberIncludeOrders) {
      return ['master', 'catalog', 'orders', 'barcode-link'];
    }
    return ['master', 'catalog', 'barcode-link'];
  }
