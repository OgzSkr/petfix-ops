...
export function getMatchingChannelsList() {
  if (!matchingChannelsCache) {
    matchingChannelsCache = [
      BENIMPOS_MASTER,
      ...listMatchingSalesChannels().map((channel) => ({
        id: channel.id,
        label: channel.label,
        role: 'sales',
...
export function listSalesMatchingChannels() {
  return getMatchingChannelsList().filter((channel) => channel.role === 'sales');
}
