...
import { nameSimilarityScore } from './product-matching/normalize.js';
...
function buildOrderLine(orderPackage, line, costByBarcode, orderDateTimezone = ORDER_TIMEZONE, lineContext = {}) {
  ...
  const genericLineName = /^(satış|satis|sale)$/i.test(rawLineName);
  const productName = (!rawLineName || genericLineName)
    ? (resolved.channelProductName || resolved.master?.name || rawLineName)
    : rawLineName;
...
export function buildLivePerformanceStats(rows) {
  const base = buildOrderStats(rows);
