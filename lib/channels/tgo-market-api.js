...
export async function fetchTgoJson(cfg, path, { method = 'GET', body } = {}) {
  ...
  if (!response.ok) {
    ...
    const duplicateUpdate = /tekrarlı stok fiyat|duplicate.*price|too many requests/i.test(String(message));
    const error = duplicateUpdate
      ? new Error('Trendyol Go: aynı ürün için çok sık güncelleme — 2–3 dakika bekleyip tekrar deneyin')
      : new Error(`Trendyol Go API hatası (${response.status}): ${message}`);
    error.statusCode = duplicateUpdate ? 429 : response.status;
    throw error;
  }
