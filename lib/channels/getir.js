import { ChannelAdapter } from './base-adapter.js';
import { getChannelCredentials } from './credentials.js';
import { probeGetirApi, isGetirApiConfigComplete } from './getir-api.js';
import { ORDER_SOURCES } from '../production/constants.js';

async function canListGetirOpsOrders() {
  try {
    const { bootstrapOpsHub, getOpsHubPool, isOpsHubReady } = await import('../ops-hub/bootstrap.js');
    const { readEnvFile } = await import('../env.js');
    const { paths } = await import('../config.js');
    if (!isOpsHubReady()) {
      await bootstrapOpsHub(await readEnvFile(paths.platformEnv));
    }
    return Boolean(getOpsHubPool());
  } catch {
    return false;
  }
}

/** Getir Çarşı — API poll + webhook ingest. */
export class GetirAdapter extends ChannelAdapter {
  constructor() {
    super('getir', 'Getir Çarşı');
    this.syncEnabled = true;
    this.scope = 'orders-profit';
  }

  async loadConfig(options = {}) {
    return getChannelCredentials({ channel: 'getir', branchId: options.branchId || null });
  }

  isConfigured(cfg) {
    return isGetirApiConfigComplete({
      shopId: cfg.shopId,
      username: cfg.apiUsername,
      password: cfg.apiPassword,
      baseUrl: cfg.apiBaseUrl,
      initialPassword: cfg.apiInitialPassword,
      env: cfg.apiEnv || 'prod'
    });
  }

  async healthCheck(options = {}) {
    const cfg = await this.loadConfig();
    if (!this.isConfigured(cfg)) {
      const opsOrders = await canListGetirOpsOrders();
      if (opsOrders) {
        return {
          ok: true,
          configured: true,
          syncEnabled: this.syncEnabled,
          opsOrdersOnly: true,
          message: 'Getir API eksik — Ops/webhook kayıtlarından siparişler gösteriliyor'
        };
      }
      return {
        ok: false,
        configured: false,
        syncEnabled: this.syncEnabled,
        message: 'GETIR_SHOP_ID, GETIR_API_BASE_URL, GETIR_API_USERNAME ve GETIR_API_PASSWORD gerekli'
      };
    }

    if (!options.probe) {
      return {
        ok: true,
        configured: true,
        syncEnabled: this.syncEnabled,
        message: 'Getir API bilgileri kayıtlı'
      };
    }

    const probe = await probeGetirApi({
      shopId: cfg.shopId,
      username: cfg.apiUsername,
      password: cfg.apiPassword,
      baseUrl: cfg.apiBaseUrl,
      initialPassword: cfg.apiInitialPassword,
      env: cfg.apiEnv
    });
    return {
      ok: probe.ok,
      configured: true,
      syncEnabled: this.syncEnabled,
      message: probe.message,
      details: probe.details
    };
  }

  async fetchOrders(options = {}) {
    const cfg = await this.loadConfig();
    if (!this.isConfigured(cfg)) return [];

    const { loginGetirApi, fetchGetirOrders } = await import('./getir-api.js');
    const { normalizeGetirPollOrder, unwrapGetirOrderPayload } = await import('../ops-hub/channels/getir-normalize.js');
    const { profitPackageFromGetirNormalized } = await import('./ops-orders-bridge.js');

    try {
      const session = await loginGetirApi(cfg);
      const unapproved = await fetchGetirOrders(cfg, 'unapproved', session);
      const packages = [];
      for (const row of unapproved) {
        const normalized = await normalizeGetirPollOrder(unwrapGetirOrderPayload(row), {
          shopId: cfg.shopId,
          endpointKind: 'unapproved',
          ingestSource: ORDER_SOURCES.WEBHOOK
        });
        if (normalized.ok) {
          packages.push(profitPackageFromGetirNormalized(normalized.order));
        }
      }
      return packages;
    } catch {
      // Ops poll + merge yedek kaynak; API geçici hata verirse sessizce boş dön.
      return [];
    }
  }
}
