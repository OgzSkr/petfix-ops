...
export async function fetchUberEatsOrders(cfg, options = {}) {
  ...
  let tgoPackages = [];
  try {
    tgoPackages = await fetchTgoGroceryOrderPackages(cfg, options);
  } catch {
    tgoPackages = [];
  }

  try {
    const orderPackages = await fetchOrderPackages({
      supplierId,
      headers,
      startDate,
      endDate,
    ...
    return dedupeOrderPackages([...tgoPackages, ...orderPackages]);
  } catch (error) {
    if (!error.orderAccessDenied) {
      throw error;
    }

    const settlementPackages = await fetchFinanceOrderPackages({
      supplierId,
      headers,
      startDate,
      endDate,
    ...
    return dedupeOrderPackages([...tgoPackages, ...settlementPackages]);
  }
}
...
async function fetchFinanceOrderPackages({ supplierId, headers, startDate, endDate, environment, cfg }) {
