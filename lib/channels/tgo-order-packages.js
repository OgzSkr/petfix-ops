...
import { resolveTgoLinePricing, sumTgoLineQuantity } from './tgo-line-pricing.js';
...
function mapTgoLineToProfitLine(line) {
  ...
  const stockCode = String(line?.items?.[0]?.id || line?.product?.id || '').trim();
  const { quantity, unitPrice, listUnitPrice, paidUnitPrice, lineGross, paidLineGross } = resolveTgoLinePricing(line);
