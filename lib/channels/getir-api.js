...
export function buildGetirPreparePayload(remoteOrder, options = {}) {
  ...
  for (const [index, product] of extractGetirOrderProducts(remoteOrder).entries()) {
    ...
    const lineOverride = lineOverrides[index] || null;
    const count = Number(
      lineOverride?.picked_qty ??
      lineOverride?.pickedQty ??
      lineOverride?.quantity ??
      product?.finalCount ??
      product?.count
    );
    const item = {
      id: lineId,
      newCount: Number.isFinite(count) && count > 0 ? Math.round(count) : 1
    };
    ...
    if (type === 'gr') {
      ...
      if (Number.isFinite(grams) && grams > 0) {
        item.newTotalWeight = Math.round(grams);
      }
    }
