...
import { probeGetirApi } from '../../channels/getir-api.js';
import { buildIntegrationSetupChecklist } from './integration-checklist.js';
import { prepareConfigForStorage } from './channel-secrets-crypto.js';
import { branchWebhookBasePath } from '../branches/webhook-branch-resolver.js';
import { getBranchById } from '../branches/branch-repository.js';

async function resolveBranchSlug(pool, branchId) {
  if (!pool || !branchId) return 'main';
  const branch = await getBranchById(pool, branchId);
  return branch?.slug || 'main';
}

export function buildWebhookPanel(platformEnv = {}, { branchSlug = 'main' } = {}) {
  const base = resolveOpsHubConfig(platformEnv).publicApiBaseUrl;
  const prefix = branchWebhookBasePath(branchSlug);
  return {
    baseUrl: base,
    branchSlug,
    endpoints: {
      yemeksepetiOrders: `${base}${prefix}/yemeksepeti/orders`,
      yemeksepetiCatalog: `${base}${prefix}/yemeksepeti/catalog`,
      getirOrdersNew: `${base}${prefix}/getir/orders/new`,
      getirOrdersCancelled: `${base}${prefix}/getir/orders/cancelled`,
      getirOrders: `${base}${prefix}/getir/orders`
    },
    legacyEndpoints: {
...
export async function listIntegrations(pool, options = {}) {
  ...
  const branchSlug = await resolveBranchSlug(pool, branchId);
  const webhooks = buildWebhookPanel(platformEnv, { branchSlug });
...
export async function getIntegrationDetail(pool, channel, options = {}) {
  ...
  return {
    ...
    integrationMode: row?.integration_mode || 'direct',
    config: maskChannelConfigSecrets(config),
    webhooks: buildWebhookPanel(platformEnv, { branchSlug }),
    setupChecklist,
    guide: {
      steps: meta.steps,
...
export async function saveIntegration(pool, channel, payload = {}, options = {}) {
  ...
  const stored = prepareConfigForStorage(
    normalized.config,
    existingRow?.secrets_ciphertext,
    platformEnv
  );
  ...
  const saved = await upsertBranchChannelConfig(pool, {
    ...
    secretsCiphertext: stored.secretsCiphertext,
...
export async function testIntegrationConnection(channel, config = {}, options = {}) {
  ...
  if (channel === 'getir') {
    if (!isGetirConfigComplete(merged)) {
      return { ok: false, message: 'Eksik alan: shopId, API base URL, kullanıcı adı ve şifre' };
    }
    const probe = await probeGetirApi({
      shopId: merged.shopId,
      username: merged.apiUsername,
      password: merged.apiPassword,
      baseUrl: merged.apiBaseUrl,
      initialPassword: merged.apiInitialPassword,
      env: merged.apiEnv
    });
    return {
      ok: Boolean(probe.ok),
