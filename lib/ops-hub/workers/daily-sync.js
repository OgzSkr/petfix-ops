...
export async function runDailySync(options = {}) {
  ...
  steps.backfillLineCosts = await runStep('backfill-order-line-costs', async () => {
    const platformEnv = await readEnvFile(paths.platformEnv);
    const config = resolveOpsHubConfig(platformEnv);
    if (!config.postgresEnabled) {
      return { skipped: true, reason: 'no_postgres' };
    }
    ...
    try {
      ...
      return await backfillOrderLineCosts(pool, {
        platformEnv,
        limit: Number(process.env.ORDER_LINE_COST_BACKFILL_LIMIT || 1000)
      });
    } finally {
      await closeOpsPool();
    }
