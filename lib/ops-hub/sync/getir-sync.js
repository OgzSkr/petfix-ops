...
import {
  normalizeGetirPollOrder,
  unwrapGetirOrderPayload,
  resolveGetirExternalId,
  extractGetirLines,
  findGetirProductForLine,
  resolveGetirProductFulfilledQuantity
...
import { recoverVanishedGetirUnapprovedOrders } from './getir-unapproved-gap.js';
...
async function syncGetirFulfilledPickQtyFromPayload(pool, orderId, rawPayload = {}) {
  ...
  for (const line of detail.lines) {
    const product = findGetirProductForLine(products, line, Number(line.line_index) || 0);
    if (!product) continue;

    const fulfilled = resolveGetirProductFulfilledQuantity(product);
    if (fulfilled == null) continue;

    await pool.query(
      `UPDATE ops_order_lines
       SET picked_qty = $1, updated_at = NOW()
       WHERE order_id = $2 AND line_index = $3`,
      [fulfilled, orderId, line.line_index]
    );
  }
}
...
export async function refreshDuplicateGetirOrder(pool, normalized, ingest) {
  ...
  if (normalized.order.rawPayload) {
    ...
    await syncGetirFulfilledPickQtyFromPayload(pool, ingest.orderId, normalized.order.rawPayload);
  }
...
export async function ingestNormalizedGetirPollOrder(pool, normalized, options = {}) {
...
export async function syncGetirReadOnly(pool, options = {}) {
  ...
  try {
    results.gapRecovery = await recoverVanishedGetirUnapprovedOrders(pool, {
      ...options,
      branchId,
      cfg,
      session,
    ...
    results.gapRecovery = { error: error.message };
    results.errors.push({ externalId: null, errors: [`Unapproved gap: ${error.message}`] });
  }
