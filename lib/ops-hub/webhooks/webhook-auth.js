...
export function isWebhookVerificationDisabled(platformEnv = {}) {
  // Production'da webhook doğrulaması ASLA kapatılamaz — fail-safe.
  const nodeEnv = String(
    platformEnv.NODE_ENV || process.env.NODE_ENV || 'development'
  ).toLowerCase();
  if (nodeEnv === 'production') {
    return false;
  }
  // Hem OPS_WEBHOOK_VERIFY_DISABLED hem de WEBHOOK_VERIFY_DISABLED kabul edilir
  // (dokümanlar/örnek env ikincisini kullanıyor).
  const raw =
    platformEnv.OPS_WEBHOOK_VERIFY_DISABLED ||
    process.env.OPS_WEBHOOK_VERIFY_DISABLED ||
    platformEnv.WEBHOOK_VERIFY_DISABLED ||
    process.env.WEBHOOK_VERIFY_DISABLED ||
    'false';
