...
export { MAX_PROFIT_REPORT_DAYS, enrichAnalyzedProfitRow, mapProfitReportDetailRow, mapProfitLineRow };
...
export async function buildOpsReportsProfit(pool, {
  ...
  for (const buyboxChannelId of buyboxChannels) {
    ...
    const rows = analyzeOrderPackages(packages, db, {
      ...
      productMatchingMode
    }).map((row) => enrichAnalyzedProfitRow({
      ...row,
...
function enrichAnalyzedProfitRow(row, buyboxChannelId) {
  const channelId = row.channel || buyboxChannelId || '';
  const channel = channelId ? getChannel(channelId) : null;
  return {
    ...row,
    channel: channelId,
    channelLabel: channel?.label || row.channelLabel || channelId || '—'
  };
}

function mapProfitLineRow(line = {}) {
  const unitSalesPrice = line.unitSalesPrice ?? line.unitPrice ?? 0;
  const lineSalesAmount = line.lineSalesAmount ?? line.lineTotal ?? line.lineGrossAmount ?? line.amount ?? 0;
  return {
    ...line,
    productName: line.productName || line.title || line.name || '',
    barcode: line.barcode || line.masterBarcode || '',
...
function mapProfitReportListRow(row) {
  return {
    ...
    profitConfidenceLabel: labelProfitConfidenceForRow(row)
  };
}

function mapProfitReportDetailRow(row) {
  return {
    ...mapProfitReportListRow(row),
    customerName: row.customerName || '',
    customerPhone: row.customerPhone || '',
    customerAddress: row.customerAddress || '',
    customerIdentityNumber: row.customerIdentityNumber || '',
    ...
    portalFinancials: row.portalFinancials || null,
    getirFinancials: row.getirFinancials || null,
    packageGrossAmount: row.packageGrossAmount,
    packageTotalDiscount: row.packageTotalDiscount,
    packagePortalCommissionAmount: row.packagePortalCommissionAmount,
    packageDeliveryFee: row.packageDeliveryFee,
    packagePartialRefund: row.packagePartialRefund,
    packageProvisionAmount: row.packageProvisionAmount,
    packageProvisionNet: row.packageProvisionNet,
    packageSellerRevenue: row.packageSellerRevenue,
    packageDiscountSellerRevenue: row.packageDiscountSellerRevenue,
    portalProvisionCredit: row.portalProvisionCredit,
    dataWarnings: row.dataWarnings || [],
    matchingWarnings: row.matchingWarnings || [],
    lines: (row.lines || []).map(mapProfitLineRow)
  };
}
...
export async function buildOpsOrderProfitabilityReport(pool, {
  ...
  const rows = sorted
    ...
    .map((row) => mapProfitReportListRow(row));
...
export async function fetchOpsOrderProfitDetail(pool, {
  ...
  return {
    ok: true,
    row: mapProfitReportDetailRow(hit)
  };
}
