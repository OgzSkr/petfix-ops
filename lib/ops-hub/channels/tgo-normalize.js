...
const ACTIVE_PACKAGE_STATUSES = Object.freeze([
  'Created',
  'Picking',
  'Invoiced',
  'Shipped'
...
export async function normalizeTgoGroceryPackage(pkg, options = {}) {
  ...
  for (const [index, line] of (pkg.lines || []).entries()) {
    ...
    lines.push({
      ...
      quantity,
      unitPrice: line?.price != null ? Number(line.price) / 100 : null,
  ...
  const customer = buildCustomerPayload(pkg);
...
function buildCustomerPayload(pkg) {
  const customer = pkg.customer || {};
  const shipment = pkg.shipmentAddress || {};
  return {
    name: customer.fullName || customer.firstName || customer.name || null,
    phone: customer.phone || customer.mobile || null,
    address: shipment.fullAddress || shipment.address || shipment.addressLine || null
  };
}
