...
export async function normalizeTgoGroceryPackage(pkg, options = {}) {
  ...
  for (const [index, line] of (pkg.lines || []).entries()) {
    ...
    const { quantity, unitPrice, listUnitPrice, paidUnitPrice } = resolveTgoLinePricing(line);
    lines.push({
      lineIndex: index,
      ...
      quantity,
      unitPrice,
      listUnitPrice: listUnitPrice ?? unitPrice,
  ...
  return {
    ...
    order: {
      ...
      rawPayload: {
        ...
        tgoSourceLines: Array.isArray(pkg.lines) ? pkg.lines : []
      },
