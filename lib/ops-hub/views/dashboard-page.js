...
export function renderOpsDashboardPage({ authRequired = true, auth = null } = {}) {
  const bar = renderOpsCompactBar({
    sideHtml: renderOpsStatPills([
      ...
      { id: 'kpiMatching', label: 'bekleyen eşleştirme' },
  ...
  const bodyHtml = `
    <div class="ops-dashboard-page ops-compact-page pf-unified-page">
      <div id="dashboardParityBanner" class="data-quality-banner data-quality-banner--warn" hidden></div>
      <div id="dashboardProfitNote" class="ops-order-profit-note" hidden></div>
  ...
  return renderOpsShell({
    ...
    bodyHtml,
    scripts: ['/assets/ops-dashboard.js?v=7']
  });
}
