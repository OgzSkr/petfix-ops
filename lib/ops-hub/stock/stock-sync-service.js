...
import { buildGetirStockPayload, writeGetirStock } from '../channels/getir-stock-write.js';
...
export function buildStockPushSimulation(plan, pushOptions = {}) {
  ...
  if (opsChannel === 'getir') {
    const cfg = { shopId: 'dry-run' };
    return {
      channel: 'getir',
      dryRun: true,
      mode: pushOptions.mode || 'full',
      payload: buildGetirStockPayload(items, cfg, pushOptions),
      itemCount: items.length,
      note: 'FF_STOCK_PUSH kapalı — Getir price-and-quantity çağrılmadı'
    };
  }
  return {
    channel: opsChannel,
...
async function dispatchStockPush(opsChannel, items, platformEnv, pushOptions = {}) {
  ...
  if (opsChannel === 'getir') {
    return writeGetirStock(items, platformEnv, pushOptions);
  }
