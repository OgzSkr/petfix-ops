...
export function createChannelControlService({ matchingSync, opsPollSync, workerScheduler }) {
  async function buildControlBoard() {
    ...
    const scheduler = workerScheduler ? await workerScheduler.getStatus() : null;
    ...
    return {
      ...
      actions: {
        ...
        outboxRetry: { label: 'Outbox retry', endpoint: '/api/channels/control/actions' }
      }
    };
  }
  ...
  async function runAction(action, options = {}) {
    switch (action) {
      ...
      case 'outbox-retry':
        if (!workerScheduler) {
          throw Object.assign(new Error('workerScheduler servisi yok'), { statusCode: 503 });
        }
        return workerScheduler.runOutboxRetryWorker(true);
      default:
