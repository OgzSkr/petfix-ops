/**
 * Katalog kanalları (Trendyol, WooCommerce) eşleştirme checklist — saf fonksiyon.
 */
export function buildCatalogMatchingOpsChecklist({
  ...
  const catalogSyncedAt = channelId === 'woocommerce'
    ? ms.woocommerceCatalogSyncedAt
    : channelId === 'yemeksepeti'
      ? ms.yemeksepetiCatalogSyncedAt
      : ms.trendyolCatalogSyncedAt;

  const catalogDone = channelId === 'woocommerce'
    ? Boolean(ms.woocommerceCatalogSyncedAt)
    : channelId === 'yemeksepeti'
