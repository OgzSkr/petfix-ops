import { yemeksepetiCatalogStatusLabel } from '../../product-matching/ys-catalog-resume.js';
...
export function buildCatalogMatchingOpsChecklist({
  ...
  const ysIngest = ms.yemeksepetiCatalogIngest || {};
  ...
  const catalogDetail = channelId === 'yemeksepeti' && ms.yemeksepetiCatalogTruncated
    ? yemeksepetiCatalogStatusLabel({
      ingestedAt: ms.yemeksepetiCatalogSyncedAt,
      prepared: ms.yemeksepetiCatalogProductCount,
      truncated: true,
      lastFetchedPage: ysIngest.lastFetchedPage,
      totalPages: ms.yemeksepetiCatalogPages
    })
    : (catalogDone
  ...
  return [
    ...
    },
    {
      ...
      done: catalogDone && !(channelId === 'yemeksepeti' && ms.yemeksepetiCatalogTruncated),
