...
async function syncGetirLivePoll() {
  try {
    const { syncGetirReadOnly } = await import('../../ops-hub/sync/getir-sync.js');
    const { readEnvFile } = await import('../../env.js');
    const { paths } = await import('../../config.js');
    const platformEnv = await readEnvFile(paths.platformEnv);
    const pool = await ensureOpsHubPool(platformEnv);
    if (!pool) return null;
    return syncGetirReadOnly(pool, { platformEnv, shadowMode: true });
  } catch (error) {
    log.warn(`Getir canlı poll atlandı: ${error.message}`);
    return null;
  }
}

async function backfillGetirDeliveredHistory(fetchOptions) {
  try {
    const { syncGetirDeliveredHistory } = await import('../../ops-hub/sync/getir-sync.js');
    const { readEnvFile } = await import('../../env.js');
    const { paths } = await import('../../config.js');
    const platformEnv = await readEnvFile(paths.platformEnv);
    const pool = await ensureOpsHubPool(platformEnv);
    ...
    return syncGetirDeliveredHistory(pool, {
      platformEnv,
      days,
      shadowMode: true
    });
  } catch (error) {
    log.warn(`Getir geçmiş senkronu atlandı: ${error.message}`);
    return null;
  }
}
...
export function createChannelOrdersService({ runtime, config = {} }) {
  async function listChannelOrders(channelId, searchParams) {
    ...
    if (channelId === 'getir' && !opsOrdersOnly) {
      await syncGetirLivePoll();
    }

    if (channelId === 'getir' && force && !opsOrdersOnly) {
      await backfillGetirDeliveredHistory(fetchOptions);
    }
