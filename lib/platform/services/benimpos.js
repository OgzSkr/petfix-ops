...
import { summarizeUberOrderFinancials, summarizeGetirOrderFinancials } from '../../benimpos/channel-sale-financials.js';
...
import { fetchUberEatsOrderPackageByNumber, orderPackageHasSettlementFinancials } from '../../channels/uber-eats-orders.js';
import { fetchGetirOrderPackageByNumber, orderPackageHasGetirFinancials } from '../../channels/getir-orders.js';
...
async function fetchChannelOrderPackage(channelId, orderNumber, days, extra = {}) {
  ...
  if (channelId === 'uber-eats') {
    return fetchUberEatsOrderPackageByNumber(cfg, orderNumber, {
      days,
      orderDateMs: extra.orderDateMs || extra.orderDate || null
    });
  }

  if (channelId === 'getir') {
    return fetchGetirOrderPackageByNumber(cfg, orderNumber, {
      days,
...
export function createBenimposService({ config = {} } = {}) {
  ...
  async function previewChannelSale(payload = {}) {
    ...
    const orderPackage = await fetchChannelOrderPackage(channelId, orderNumber, days, {
      orderDateMs: payload.orderDateMs,
    ...
    const financials = channelId === 'uber-eats'
    ...
    return {
      ...
      channelReadiness,
      financials,
      settlementFinancialsLoaded: channelId === 'uber-eats'
        ? orderPackageHasSettlementFinancials(orderPackage)
        : channelId === 'getir'
          ? orderPackageHasGetirFinancials(orderPackage)
          : null,
      ...preview,
