...
import { buildChannelSaleFromOrder, createSale } from '../../benimpos/sales-create.js';
...
export function createBenimposService({ config = {} } = {}) {
  ...
  async function createChannelSale(payload = {}) {
    ...
    const result = await createSale(client, saleOrder, { dryRun });
    ...
    return {
      ...
      orderNumber,
      salesCode: result.salesCode || null,
      message: result.message || (dryRun ? 'Dry-run payload hazır.' : 'Satış oluşturuldu.'),
