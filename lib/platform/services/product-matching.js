...
import { syncChannelCostsFromMasterProducts } from '../../product-matching/master-cost-sync.js';
...
} from '../../product-matching/ys-catalog-resume.js';
import { autoApplyAllCleanupSuggestions } from '../../product-matching/post-sync-cleanup.js';
import {
...
export function createProductMatchingService() {
  ...
  async function getStatus() {
    ...
    return {
      ...
      yemeksepetiCatalogTruncated: pm.meta.channelIngest?.yemeksepeti?.truncated === true,
      yemeksepetiCatalogNextPage: pm.meta.channelIngest?.yemeksepeti?.nextPage || null,
      yemeksepetiCatalogComplete: pm.meta.channelIngest?.yemeksepeti?.catalogComplete !== false,
      yemeksepetiCatalogIngest: pm.meta.channelIngest?.yemeksepeti || null,
  ...
  async function syncMasterFromBenimpos(options = {}) {
    ...
    const costSync = syncChannelCostsFromMasterProducts(db, { sourceName: 'master sync' });

    const autoCleanup = autoApplyAllCleanupSuggestions(db, pm);
  ...
  async function syncYemeksepetiCatalogProducts(options = {}) {
    ...
    const db = await readDb();
    const pm = getProductMatching(db);
    const catalogOpts = resolveYemeksepetiCatalogSyncOptions(pm, options);
    ...
    persistYemeksepetiCatalogResume(pm, ingest.summary);
    const autoCleanup = autoApplyAllCleanupSuggestions(db, pm, { channelId: 'yemeksepeti' });
  ...
  async function runPostSyncCleanup(options = {}) {
    const db = await readDb();
    const pm = getProductMatching(db);
    const result = autoApplyAllCleanupSuggestions(db, pm, options);
    if (result.removedMappings > 0 || result.prunedMasters > 0) {
      markWorkbenchStoresDirty(pm);
      db.meta = db.meta || {};
      db.meta.updatedAt = new Date().toISOString();
      await writeDb(db);
