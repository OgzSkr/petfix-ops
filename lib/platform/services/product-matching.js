...
export function createProductMatchingService() {
  ...
  function getMasterSyncStatus() {
    return {
      running: masterSyncJob.running,
      startedAt: masterSyncJob.startedAt,
      finishedAt: masterSyncJob.finishedAt,
      result: masterSyncJob.result,
      error: masterSyncJob.error,
      progress: masterSyncJob.progress
    };
  }
  ...
  async function syncMasterFromBenimpos(options = {}) {
    ...
    onProgress({ phase: 'save', message: 'Veritabanı güncelleniyor…' });
    const db = await readDb();
    const pm = getProductMatching(db);
  ...
  async function syncGetirCatalogProducts(options = {}) {
    ...
    pm.meta.channelIngest = pm.meta.channelIngest || {};
    pm.meta.channelIngest['getir-catalog'] = ingest.summary;
    db.meta = db.meta || {};
    db.meta.updatedAt = new Date().toISOString();
    await writeDb(db);
