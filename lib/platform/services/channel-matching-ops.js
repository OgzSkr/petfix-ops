      ...
      try {
        if (step === 'master') {
          ...
          if (channelId === 'trendyol-marketplace') {
            results.catalog = await productMatching.syncTrendyolCatalogProducts();
          } else if (channelId === 'woocommerce') {
            results.catalog = await productMatching.syncWooCommerceCatalogProducts(payload.catalog || {});
          } else if (channelId === 'yemeksepeti') {
