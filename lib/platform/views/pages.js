...
export function createPageViews(auth, runtimeConfig = {}) {
  ...
  function renderOtherChannelCostsPage() {
    const bootstrap = jsonForHtml({
      authRequired: Boolean(auth.isEnabled()),
      costScope: 'other-channels',
      costScopeLabel: 'Diğer Kanallar'
    });

    const innerHtml = `
      <div class="products-wrap pf-products-costs">
        <section class="panel channel-costs-intro">
          <h2>Diğer Kanal Maliyetleri</h2>
          <p class="platform-lead">Uber Eats, Getir, Yemeksepeti ve WooCommerce sipariş kârlılığı bu maliyet setini kullanır. Trendyol Pazaryeri maliyetleri <a href="/marketplace/products">Ürün Ayarları</a> sayfasındadır — birbirinden bağımsızdır.</p>
        </section>
        <form id="filterForm" class="filter-card">
          <div class="filter-grid">
            <div><label>Ürün adı</label><input name="title" type="search" placeholder="Ürün adı"></div>
        ...
        <section class="settings-card">
          <div class="settings-head">
            <h2>Maliyet Tablosu</h2>
          </div>
          <div class="settings-toolbar">
            <button type="button" class="btn-brown" id="refreshData">Verileri Güncelle</button>
            <button type="button" class="btn-green" id="exportCsv">Excel Dosyasını İndir</button>
            <span class="settings-toolbar-hint">Maliyet değişikliklerini satırdaki <strong>Güncelle</strong> ile kaydedin.</span>
          </div>
          <div class="products-table-wrap" id="productsTableWrap">
            <table class="products-table">
              <thead>
    ...
    return wrapPanelPage({
      title: 'Diğer Kanal Maliyetleri',
      activeModule: 'products',
      activeItem: 'channel-costs',
      auth,
      bodyClass: 'products-page channel-costs-page pf-products-page',
      bodyHtml: innerHtml,
      stylesheets: ['/assets/styles.css', '/assets/products.css'],
      scripts: ['/assets/products.js?v=3']
    });
  ...
  function renderChannelCostsPage() {
    return renderOtherChannelCostsPage();
  }
  ...
  return {
    renderProductsPage,
    renderOtherChannelCostsPage,
    renderChannelCostsPage,
