...
export function renderAdminUsersPage({ authRequired = true, auth = null } = {}) {
  ...
  const bar = renderOpsCompactBar({
    sideHtml: `${renderOpsStatPills([
      { id: 'staffStatTotal', label: 'personel' },
      { id: 'staffStatActive', label: 'aktif' },
      { id: 'staffStatSessions', label: 'oturum', muted: true, valueClass: 'ops-compact-stat-value--sm' }
    ])}${infoBlocks}`
  });

  const bodyHtml = `
    <div class="ops-staff-page ops-compact-page pf-unified-page">
      <section class="ops-panel ops-compact-page-panel">
        ${bar}
        <p class="ops-order-profit-note">Depo, kurye ve süpervizör hesapları HzlMrktOps mobil uygulamasına giriş yapar.</p>
      </section>
