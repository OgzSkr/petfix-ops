...
export function renderHzlMrktOpsProfitPage({ auth, productMatchingMode = 'legacy' }) {
  ...
  const innerHtml = `
      <div class="orders-wrap pf-ops-orders">
        ...
        <nav class="orders-lifecycle-tabs" id="ordersLifecycleTabs" role="tablist" aria-label="Sipariş durumu">
          <button type="button" class="orders-lifecycle-tab" data-lifecycle="active" role="tab" aria-selected="false">Aktif Siparişler <span class="orders-lifecycle-count" id="lifecycleCountActive"></span></button>
          <button type="button" class="orders-lifecycle-tab active" data-lifecycle="completed" role="tab" aria-selected="true">Tamamlanmış Siparişler <span class="orders-lifecycle-count" id="lifecycleCountCompleted"></span></button>
        </nav>
  ...
  return wrapPanelPage({
    ...
    bodyHtml: innerHtml,
    stylesheets: ['/assets/styles.css?v=corp2', '/assets/orders.css?v=24'],
    scripts: ['/assets/channel-logos.js?v=2', '/assets/orders.js?v=27']
  });
}
