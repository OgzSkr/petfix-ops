...
export function renderChannelOrdersPage({ channelId, auth, productMatchingMode = 'legacy' }) {
  ...
  return wrapPanelPage({
    ...
    scripts: ['/assets/channel-page.js?v=3', '/assets/orders.js?v=45']
  });
}
...
export function renderHzlMrktOpsProfitPage({ auth, productMatchingMode = 'legacy' }) {
  ...
  const ordersInfoBlocks = [
    renderOpsInfoDisclosure({
      id: 'ordersInfoLifecycle',
      title: 'Aktif ve tamamlanan',
      items: [
        'Aktif: henüz teslim edilmemiş veya iptal olmamış siparişler.',
        'Tamamlanmış: teslim edilen veya sonlanan siparişler.',
        'Aktif sekmede liste yaklaşık 45 saniyede bir sessiz yenilenir.'
      ]
    }),
    renderOpsInfoDisclosure({
      id: 'ordersInfoBenimpos',
      title: 'BenimPOS ve kasa',
      paragraphs: [
        'Kasa işlemi sipariş detayından yapılır. Otomatik aktarım durumu tabloda BenimPOS sütununda görünür.'
      ]
    })
  ].join('');
  ...
  const innerHtml = `
      <div class="orders-wrap pf-ops-orders ops-orders-page pf-unified-page">
        <div class="ops-orders-hero">
          <div class="ops-orders-hero-copy">
            <p class="ops-analytics-eyebrow">Operasyon · Siparişler</p>
            <h1 class="ops-orders-title">Sipariş takibi</h1>
            <p class="ops-orders-lead">Bugünkü ve geçmiş siparişleri kanal ve döneme göre filtreleyin. Aktif siparişler üst sekmede; kasa işlemi detay ekranından yapılır.</p>
            <div class="ops-info-stack ops-info-stack--compact">${ordersInfoBlocks}</div>
          </div>
          <div class="ops-orders-stats">
            <div class="ops-orders-stat">
              <span class="ops-orders-stat-value" id="ordersHeroActive">—</span>
              <span class="ops-orders-stat-label">Aktif sipariş</span>
            </div>
            <div class="ops-orders-stat">
              <span class="ops-orders-stat-value" id="ordersHeroCompleted">—</span>
              <span class="ops-orders-stat-label">Tamamlanan</span>
            </div>
            <div class="ops-orders-stat ops-orders-stat--muted">
              <span class="ops-orders-stat-value ops-orders-stat-value--sm" id="ordersHeroPeriod">Bugün</span>
              <span class="ops-orders-stat-label">Seçili dönem</span>
            </div>
          </div>
        </div>

        <section class="ops-panel ops-orders-toolbar-panel">
        ...
        <form id="ordersFilterForm" class="ops-panel ops-orders-filter-panel orders-filter-card orders-filter-card--compact">
          <header class="ops-panel-head">
            <div>
              <h3>Filtreler</h3>
              <p class="ops-panel-sub">Dönem ve sipariş durumu</p>
            </div>
          </header>
          <div class="ops-orders-filter-body">
            <div class="orders-filter-grid">
              <div>
                <label for="daysSelect">Dönem</label>
                <select id="daysSelect" name="days">
        ...
        <section class="ops-panel ops-orders-table-panel orders-card" id="ordersListPanel">
          <header class="ops-panel-head ops-panel-head--split">
            <div>
              <h3 id="ordersListTitle">Aktif siparişler</h3>
              <p class="ops-panel-sub" id="ordersListSub">Seçili kanal ve dönemdeki siparişler</p>
            </div>
          </header>
  ...
  return wrapPanelPage({
    ...
    stylesheets: [
      ...
      '/assets/orders.css?v=28',
      '/assets/panel-unified.css?v=4'
    ],
    scripts: ['/assets/channel-logos.js?v=2', '/assets/orders.js?v=45']
  });
}
