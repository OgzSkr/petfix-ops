...
import { isHzlmrktopsBuyboxChannel, HZLMRKTOPS_BUYBOX_CHANNEL_IDS } from '../../hzlmrktops/constants.js';
...
export function renderChannelOrdersPage({ channelId, auth, productMatchingMode = 'legacy' }) {
          ...
          <div class="orders-filter-grid">
            <div>
              ...
              <select id="daysSelect" name="days">
                <option value="1" selected>Bugün</option>
  ...
  return wrapPanelPage({
    ...
    scripts: ['/assets/channel-page.js?v=3', '/assets/orders.js?v=46']
  });
}
...
export function renderHzlMrktOpsProfitPage({ auth, productMatchingMode = 'legacy' }) {
  ...
  const channelTabs = [
    ...
    ...HZLMRKTOPS_BUYBOX_CHANNEL_IDS.map((channelId) => {
      const channel = getChannel(channelId);
  ...
  const ordersInfoBlocks = renderOpsInfoDisclosure({
    id: 'ordersInfo',
    title: 'Sipariş notu',
    items: [
      'Aktif: henüz sonlanmamış siparişler; liste yaklaşık 90 saniyede bir yenilenir.',
      'Tamamlanmış: teslim edilen veya sonlanan siparişler.',
      'Kasa işlemi sipariş detayından yapılır; BenimPOS durumu tabloda görünür.'
    ]
  });

  const innerHtml = `
      <div class="orders-wrap pf-ops-orders ops-orders-page pf-unified-page">
        <section class="ops-panel ops-orders-panel" id="ordersListPanel">
          <header class="ops-order-profit-bar ops-orders-bar">
            <div class="ops-order-profit-bar-main ops-orders-bar-main">
              <nav class="orders-subnav ops-orders-channel-nav" id="hzlmrktopsChannelFilters" role="tablist" aria-label="Kanal filtresi">
                ${channelTabs.map((tab, index) => {
                  ...
                  return `<button type="button" class="orders-subnav-tab orders-subnav-tab--channel${index === 0 ? ' active' : ''}" data-channel="${escapeHtml(tab.id)}" data-label="${escapeHtml(tab.label)}" role="tab" aria-selected="${index === 0 ? 'true' : 'false'}" title="${escapeHtml(tab.label)}">${tab.id === 'all' ? '<span class="orders-subnav-label">Tümü</span>' : renderChannelLogo(tab.id, { size: 'sm' })}<span class="orders-subnav-count" data-count-for="${escapeHtml(tab.id)}"></span></button>`;
                }).join('')}
              </nav>
              <nav class="orders-lifecycle-tabs ops-orders-lifecycle-nav" id="ordersLifecycleTabs" role="tablist" aria-label="Sipariş durumu">
                <button type="button" class="orders-lifecycle-tab active" data-lifecycle="active" role="tab" aria-selected="true">Aktif <span class="orders-lifecycle-count" id="lifecycleCountActive"></span></button>
                <button type="button" class="orders-lifecycle-tab" data-lifecycle="completed" role="tab" aria-selected="false">Tamamlanmış <span class="orders-lifecycle-count" id="lifecycleCountCompleted"></span></button>
              </nav>
            </div>
            <div class="ops-order-profit-bar-side ops-orders-bar-side">
              <div class="ops-order-profit-counts ops-orders-counts">
                <span class="ops-order-profit-count"><strong id="ordersHeroActive">—</strong> aktif</span>
                <span class="ops-order-profit-count"><strong id="ordersHeroCompleted">—</strong> tamamlanan</span>
                <span class="ops-order-profit-count ops-order-profit-count--muted" id="ordersHeroPeriod">Bugün</span>
              </div>
              ${ordersInfoBlocks}
            </div>
          </header>

          <form id="ordersFilterForm" class="ops-orders-filter-inline orders-filter-card orders-filter-card--compact">
            <div class="ops-orders-filter-row orders-filter-grid">
              <label class="ops-orders-filter-field" for="daysSelect">
                <span>Dönem</span>
                <select id="daysSelect" name="days">
                  <option value="1" selected>Bugün</option>
                  <option value="7">Son 7 gün</option>
              ...
              <label class="ops-orders-filter-field custom-date-field" for="startDate">
                <span>Başlangıç</span>
                <input type="date" id="startDate" name="startDate" disabled>
              </label>
              <label class="ops-orders-filter-field custom-date-field" for="endDate">
                <span>Bitiş</span>
                <input type="date" id="endDate" name="endDate" disabled>
              </label>
              <label class="ops-orders-filter-field" for="statusFilter">
                <span>Durum</span>
                <select id="statusFilter" name="status">
                  <option value="">Tüm durumlar</option>
                </select>
              ...
              <div class="orders-filter-actions ops-orders-filter-actions">
                <button type="button" class="btn-coral" id="clearOrderFilters">Temizle</button>
                <button type="submit" class="btn-brown">Uygula</button>
              </div>
            </div>
          </form>

          <span id="ordersListTitle" class="visually-hidden">Aktif siparişler</span>

          <div class="orders-cache-banner" id="ordersCacheBanner" hidden aria-live="polite"></div>
          <div class="orders-sync-banner" id="ordersSyncBanner" hidden aria-live="polite"></div>

          <div class="orders-toolbar ops-orders-table-toolbar">
            <input type="search" id="ordersSearch" class="orders-search" placeholder="Sipariş no, müşteri veya ürün ara…" aria-label="Sipariş ara" autocomplete="off">
            <button type="button" class="btn-brown orders-sound-toggle is-on" id="ordersSoundToggle" aria-pressed="true" title="Yeni aktif sipariş geldiğinde ses çal">Ses: Açık</button>
            <button type="button" class="btn-brown" id="refreshOrders">Güncelle</button>
          </div>
      ...
      <div class="orders-modal-backdrop" id="orderModalBackdrop" hidden>
        <div class="orders-modal orders-modal--ops" role="dialog" aria-modal="true" aria-labelledby="orderModalTitle">
          <div class="orders-modal-head">
            <h3 id="orderModalTitle">Sipariş Detayı</h3>
            <button type="button" class="orders-modal-close" id="orderModalClose" aria-label="Kapat">×</button>
          </div>
  ...
  return wrapPanelPage({
    ...
    bodyClass: 'orders-page ops-orders-page pf-channel-page pf-unified-page',
    bodyHtml: innerHtml,
    stylesheets: [
      '/assets/ops-tokens.css?v=2',
      '/assets/ops-components.css?v=corp2',
      '/assets/styles.css?v=corp3',
      '/assets/orders.css?v=29',
      '/assets/panel-unified.css?v=5'
    ],
    scripts: ['/assets/channel-logos.js?v=2', '/assets/orders.js?v=48']
  });
}
