...
export function createRouteHandler(deps) {
  return async function handleRequest(request, response) {
    ...
    try {
      ...
      if (statusCode >= 500) {
        log.error(`${request.method} ${path} → ${statusCode}: ${error.stack || error.message}`, meta);
      } else if (statusCode >= 400) {
        log.warn(`${request.method} ${path} → ${statusCode}: ${error.message}`, meta);
      }

      // 5xx'te iç hata detayını/stack'i istemciye sızdırma; 4xx kasıtlı doğrulama mesajıdır.
      const clientMessage = statusCode >= 500 ? 'Sunucu hatası' : error.message || 'İstek işlenemedi';
      sendJson(response, { error: clientMessage }, statusCode);
    }
  };
}
