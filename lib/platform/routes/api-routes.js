...
export async function handleApiRoutes(ctx) {
  ...
  if (request.method === 'GET' && (url.pathname === '/api/hzlmrktops/orders' || url.pathname === '/api/marketnext/orders')) {
    auth.assertAuthorized(request);
    await sendJson(response, await marketNextOrders.listHzlMrktOpsOrders(url.searchParams));
    return true;
  }

  if (request.method === 'GET' && (url.pathname === '/api/hzlmrktops/orders/export' || url.pathname === '/api/marketnext/orders/export')) {
    auth.assertAuthorized(request);
    await marketNextOrders.sendHzlMrktOpsOrdersCsvExport(response, url.searchParams);
    return true;
  }
