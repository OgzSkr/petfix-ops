...
export async function handleApiRoutes(ctx) {
  ...
  if (request.method === 'GET' && url.pathname === '/api/workers/status') {
    ...
    if (!workerScheduler) {
      await sendJson(response, { ok: false, error: 'Worker scheduler yok' }, 503);
      return true;
    }
    await sendJson(response, await workerScheduler.getStatus());
    return true;
  }

  if (request.method === 'POST' && url.pathname === '/api/workers/outbox-retry') {
    auth.assertAuthorized(request);
    if (!workerScheduler) {
      await sendJson(response, { ok: false, error: 'Worker scheduler yok' }, 503);
      return true;
    }
    await sendJson(response, await workerScheduler.runOutboxRetryWorker(true));
    return true;
  }
  ...
  if (request.method === 'GET' && url.pathname === '/api/dashboard/summary') {
    auth.assertAuthorized(request);
    await sendJson(response, await dashboardSummary.buildDashboardSummary(url.searchParams));
    return true;
  }
