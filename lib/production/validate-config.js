...
export function validateProductionConfig(platformEnv = {}, processEnv = process.env) {
  ...
  const opsPostgres = read(processEnv, platformEnv, 'OPS_POSTGRES_URL');
  if (opsPostgres && !encryptionKey) {
    missing.push('Ops Hub kanal secret şifreleme için ENCRYPTION_KEY');
  }
