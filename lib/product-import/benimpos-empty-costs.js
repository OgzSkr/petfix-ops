...
export function mergeBenimposEmptyCosts(db, items = [], { sourceName = 'BenimPOS' } = {}) {
  ...
  const costByBarcode = new Map(db.channelCosts.map((cost) => [String(cost.barcode || ''), cost]));
  const masterBarcodes = new Set(
    (db.productMatching?.masterProducts || [])
      .map((product) => String(product.benimposBarcode || product.barcode || '').trim())
      .filter(Boolean)
  );
  ...
  for (const cost of db.channelCosts) {
...
export function findByBarcodeInCosts(db, barcode) {
  return findByBarcode(db.channelCosts || [], barcode);
}
