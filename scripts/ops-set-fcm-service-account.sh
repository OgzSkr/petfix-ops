...
# Host path — docker /app/data volume mount
REMOTE_PATH="${VPS_FCM_SA_PATH:-/opt/petfix/buybox-platform/data/fcm-service-account.json}"
CONTAINER_FCM_PATH="/app/data/fcm-service-account.json"
...
"${SSH[@]}" "chmod 644 ${REMOTE_PATH} && chown petfix:petfix ${REMOTE_PATH} 2>/dev/null || chmod 644 ${REMOTE_PATH}"

"${SSH[@]}" bash -s <<EOF
set -euo pipefail
cd "${APP_DIR}"
if grep -q '^FCM_SERVICE_ACCOUNT_PATH=' "${ENV_FILE}"; then
  sed -i "s|^FCM_SERVICE_ACCOUNT_PATH=.*|FCM_SERVICE_ACCOUNT_PATH=${CONTAINER_FCM_PATH}|" "${ENV_FILE}"
else
  echo "FCM_SERVICE_ACCOUNT_PATH=${CONTAINER_FCM_PATH}" >> "${ENV_FILE}"
fi
