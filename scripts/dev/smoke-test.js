...
import { resolveDeployProfile, DEPLOY_PROFILE } from '../../lib/production/deploy-profile.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const root = path.resolve(__dirname, '../..');
const envPath = process.env.PETFIX_ENV_FILE
  ? path.join(root, process.env.PETFIX_ENV_FILE)
  : path.join(root, '.env');
...
async function main() {
  const checks = [];
  const deployProfile = resolveDeployProfile({ DEPLOY_PROFILE: readEnvValue('DEPLOY_PROFILE') });
  let opsOnly = deployProfile === DEPLOY_PROFILE.OPS_ONLY;
  {
    const marketplaceProbe = await fetch(`${base}/api/dashboard`);
    if (marketplaceProbe.status === 404) opsOnly = true;
  }
  ...
  if (opsOnly) {
    const marketplacePage = await fetch(`${base}/marketplace/trendyol`, { redirect: 'manual' });
    if (marketplacePage.status !== 302 && marketplacePage.status !== 404) {
      throw new Error(`marketplace page blocked: expected 302 or 404, got ${marketplacePage.status}`);
    }
    checks.push('ops-only-marketplace-blocked');
    await check('buybox api blocked', `${base}/api/buybox/analytics?days=7`, 404);
    checks.push('ops-only-buybox-blocked');
  } else {
  ...
  if (token) {
    ...
    if (!opsOnly) {
      ...
      await check('ops branches api', `${base}/api/ops/branches`, 200, { headers });
      checks.push('ops-branches-api');
    }
    ...
    if (!opsOnly) {
      ...
      await check('channels summary', `${base}/api/dashboard/channels-summary`, 200, { headers });
  ...
  if (opsOnly) {
    const dashResponse = await fetch(`${base}/dashboard`, { redirect: 'manual' });
    if (dashResponse.status !== 302) {
      throw new Error(`dashboard redirect: expected 302, got ${dashResponse.status}`);
    }
    checks.push('dashboard-redirect-hzlmrktops');
  } else {
